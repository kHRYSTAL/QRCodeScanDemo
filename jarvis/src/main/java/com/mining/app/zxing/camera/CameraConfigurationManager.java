...
final class CameraConfigurationManager {
  ...
  void initFromCameraParameters(Camera camera) {
     ...
     * cameraResolution = findBestPreviewSizeValue((CharSequence) parameters, new Point(width,
     * height));
  ...
  private static Point findBestPreviewSizeValue(CharSequence previewSizeValueString,
    ...
    for (String previewSize : COMMA_PATTERN.split(previewSizeValueString)) {
      ...
      int newDiff = Math.abs(newY - screenResolution.y) + Math.abs(newX - screenResolution.x);
      if (newDiff == 0) {
        bestX = newX;
        bestY = newY;
        break;
